        ...
        <>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            {loadingBookings ? (
              ...
              <div style={{ fontSize: 12, color: "var(--muted)", padding: "12px", textAlign: "center", background: "rgba(124,58,237,0.05)", borderRadius: 8, border: "1px dashed var(--border)" }}>
                No upcoming bookings.
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 280, overflowY: "auto" }}>
                {bookings.map(b => (
                  <div key={b.id} style={{ padding: "10px 12px", background: "rgba(124,58,237,0.08)", borderRadius: 8, border: "1px solid rgba(124,58,237,0.2)" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--fg)", marginBottom: 3 }}>{b.title}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>🕐 {formatDateTime(b.start)}</div>
                    {b.attendees && b.attendees.length > 0 && (
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>👤 {b.attendees.join(", ")}</div>
                    )}
                    {b.html_link && (
                      <a href={b.html_link} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: "#a78bfa", textDecoration: "none", marginTop: 4, display: "inline-block" }}>Open in Calendar ↗</a>
                    )}
                  </div>
                ))}
              </div>
